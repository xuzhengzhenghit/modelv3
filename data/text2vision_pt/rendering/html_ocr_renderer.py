...
_PREPROC_DIR = _Path(__file__).resolve().parent.parent / "preprocessing"
...
#content { width: 100%; height: auto; overflow: visible; display: flow-root; }
#paper.finalized #content { height: 100%; }
p { margin: 0 0 var(--paragraph-gap) 0; text-align: var(--text-align); }
.display-math { margin: 0.45em 0 0.65em 0; text-align: center; overflow: visible; }
...
.katex-display { margin: 0 !important; overflow: visible; }
</style>
</head>
<body>
...
window.layoutBlocks = async function(payload) {
  const paper = document.getElementById('paper');
  const content = document.getElementById('content');
  paper.classList.remove('finalized');

  applyStyle(payload.style, payload.style.font_size);
  paper.style.width = payload.style.width + 'px';
  ...
  for (let i = 0; i < (payload.blocks || []).length; i++) {
    ...
    appendBlock(content, block, state);
    const afterCount = content.children.length;

    const bid = block.id || ('block-' + i);
    for (let j = beforeCount; j < afterCount; j++) {
      content.children[j].dataset.blockId = bid;
    }
    renderedBlockIds.push(bid);
  }
  ...
  var blockRects = [];
  var horizontalOverflow = content.scrollWidth > content.clientWidth + 1;
  ...
  for (var k = 0; k < children.length; k++) {
    ...
    blockRects.push(br);
    if (br.left < -1 || br.right > content.clientWidth + 1 || el.scrollWidth > el.clientWidth + 1) {
      horizontalOverflow = true;
    }
  }

  return {
    contentHeight: Math.ceil(content.scrollHeight),
    contentWidth: Math.ceil(content.scrollWidth),
    clientWidth: content.clientWidth,
    horizontalOverflow: horizontalOverflow,
    renderedBlockIds: renderedBlockIds,
    blockRects: blockRects,
    katexErrors: state.katexErrors,
    katexMissing: state.katexMissing
  };
};

window.finalizePaper = function(height) {
  var paper = document.getElementById('paper');
  var content = document.getElementById('content');
  paper.style.height = height + 'px';
  paper.style.overflow = 'hidden';
  paper.classList.add('finalized');
...
// ── Legacy renderBlocks (kept for backward compat with render()) ──
...
class RenderConfig:
    ...
    # Dynamic canvas buckets (all multiples of patch_size)
    width_buckets: tuple[int, ...] = (384, 512, 768, 1024)
    height_buckets: tuple[int, ...] = (64, 96, 128, 160, 192, 256, 320, 384, 448, 512)
...
@dataclasses.dataclass(frozen=True)
class RenderUnit:
    """Immutable unit of rendering: blocks + target, never modified by renderer."""
    sample_id: str
    blocks: tuple
    target_text: str
    ...
    def block_ids(self) -> list[str]:
        return [str(b.get("id", f"block-{i}")) for i, b in enumerate(self.blocks)]


class NeedsSplit(RuntimeError):
    """Content exceeds max height — upstream paginator must split into two RenderUnits."""


class TooWide(RuntimeError):
    """Content exceeds max width even at widest bucket."""


def _choose_bucket(value: int, buckets: tuple[int, ...]) -> int:
    """Pick the first bucket >= value. Raises NeedsSplit/TooWide if no bucket fits."""
    for b in sorted(buckets):
        if value <= b:
            return b
    raise RuntimeError(f"value {value} exceeds max bucket {max(buckets)}")
...
class HtmlOCRRenderer:
    ...
    def _choose_width(self, blocks, rng: random.Random) -> list[int]:
        """Return ordered width candidates based on content type."""
        has_table = any(b.get("kind") == "table" for b in blocks)
        has_display_math = any(b.get("kind") == "math_display" for b in blocks)
        buckets = sorted(self.config.width_buckets, reverse=True)  # widest first
        if has_table:
            return [b for b in buckets if b >= 768]
        if has_display_math:
            return [b for b in buckets if b >= 512]
        return buckets
    ...
    def render_dynamic(
        ...
        """Render with dynamic canvas sizing — layout-first, single-pass design.

        Flow:
          1. Choose width bucket → single auto-height layout → measure.
          2. If horizontal overflow → wider bucket → re-layout once.
          3. Compute required height = content_h + 2*padding_y + slack.
          4. Choose height bucket → finalize paper → verify no overflow.
          5. Verify block IDs match → screenshot.

        The renderer NEVER modifies target_text, deletes blocks, or shrinks font.
        """
        import math
        ...
        width_candidates = self._choose_width(unit.blocks, rng)
        blocks_list = list(unit.blocks)
        ...
        # ── Layout: try width buckets until no horizontal overflow ──
        layout_info = None
        paper_w = None
        begin = time.perf_counter()
        ...
        try:
            for width in width_candidates:
                layout_info = page.evaluate(
                    "p => window.layoutBlocks(p)",
                    {"blocks": blocks_list, "style": {**style, "width": width, "font_size": style["font_size"]}},
                )
                if not layout_info["horizontalOverflow"]:
                    paper_w = width
                    break

            if paper_w is None:
                raise TooWide(f"Content too wide for all buckets: {unit.sample_id}")
        except TooWide:
            raise
        except Exception:
            self.close()
            ...
            layout_info = page.evaluate(
                "p => window.layoutBlocks(p)",
                {"blocks": blocks_list, "style": {**style, "width": cfg.width_buckets[-1], "font_size": style["font_size"]}},
            )
            paper_w = cfg.width_buckets[-1]

        t_layout = time.perf_counter()

        # ── Compute required height ──
        padding_y = int(style["padding_y"])
        content_h = int(layout_info["contentHeight"])
        slack = rng.choice([0, 0, 0, 0, 0, cfg.patch_size])
        required_h = content_h + 2 * padding_y + slack

        try:
            paper_h = _choose_bucket(required_h, cfg.height_buckets)
        except RuntimeError:
            raise NeedsSplit(
                f"Content too tall: required={required_h} max={max(cfg.height_buckets)}: {unit.sample_id}"
            )

        # ── Finalize paper ──
        final_info = page.evaluate("h => window.finalizePaper(h)", paper_h)
        if final_info["overflow"]:
            raise RuntimeError(f"Overflow after finalization: {unit.sample_id}")

        # ── Block ID consistency check ──
        expected_ids = unit.block_ids()
        if layout_info["renderedBlockIds"] != expected_ids:
            raise RuntimeError(
                f"Block ID mismatch: rendered={layout_info['renderedBlockIds']} "
                f"expected={expected_ids} for {unit.sample_id}"
            )

        # ── Screenshot ──
        screenshot_kwargs: dict[str, Any] = {"type": cfg.screenshot_type, "animations": "disabled"}
        if cfg.screenshot_type == "jpeg":
            screenshot_kwargs["quality"] = cfg.jpeg_quality
        screenshot = page.locator("#paper").screenshot(**screenshot_kwargs)
        ...
        grid_h = paper_h // cfg.patch_size
        grid_w = paper_w // cfg.patch_size

        return {
            "pixel_values": pixel_values,
            "target_text": unit.target_text,  # NEVER modified by renderer
            "image_grid_thw": (1, grid_h, grid_w),
            "num_visual_tokens": grid_h * grid_w,
            "png_or_jpeg_bytes": screenshot,
            "paper_size": (paper_w, paper_h),
            "content_size": (int(layout_info["contentWidth"]), content_h),
            "render_meta": {
                **layout_info,
                **final_info,
                "paper_w": paper_w,
                ...
                "style": style,
                "katex_available": self._katex_dist is not None,
                "layout_ms": (t_layout - begin) * 1000,
                "screenshot_ms": (t_screenshot - t_layout) * 1000,
                "decode_ms": (t_decode - t_screenshot) * 1000,
                "total_ms": (t_decode - begin) * 1000,
            },
        }
